...
class HelpfulsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "valid create helpful with group" do
    ...
    #参考になるによってお知らせ、参考になる、ポイント履歴の数が1つ増える。レビューの書き手は2つのグループに所属しているのでGroupPointHistoryは2つ増える
  ...
  test "valid create helpful without group" do
    login_as(@user2, scope: :user)
    #参考になるによってお知らせ、参考になる、ポイント履歴の数が1つ増えるがgroupに未所属なのでgroupPointHistroyは増えない
