...
class ZoomsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "zoom create without title" do
    ...
    assert_no_difference 'Zoom.count' do
      post zooms_path, params: { zoom: { title: "", join_url: "https://us04web.zoom.us/j/73988743960?pwd=cWRNMzNGcUwrWFlyVFg3ZjlRUjF3Zz09", user_id: @user.id, start_time: Time.now+100, end_time: Time.now+200, count: 1}}
    end
  ...
  test "zoom create without join_url" do
    login_as(@user, scope: :user)
    assert_no_difference 'Zoom.count' do
      post zooms_path, params: { zoom: { title: "test1", join_url: "", user_id: @user.id, start_time: Time.now+100, end_time: Time.now+200, count: 1}}
    end
    assert_template 'zooms/new'
  end
  ...
  test "zoom create without vaild start and end time" do
    login_as(@user, scope: :user)
    assert_no_difference 'Zoom.count' do
      post zooms_path, params: { zoom: { title: "test1", join_url: "https://us04web.zoom.us/j/73988743960?pwd=cWRNMzNGcUwrWFlyVFg3ZjlRUjF3Zz09",user_id: @user.id, start_time: Time.now+200, end_time: Time.now+100, count: 1}}
    end
    assert_template 'zooms/new'
  end
  ...
  test "zoom create without vaild start time" do
    login_as(@user, scope: :user)
    assert_no_difference 'Zoom.count' do
      post zooms_path, params: { zoom: { title: "test1", join_url: "https://us04web.zoom.us/j/73988743960?pwd=cWRNMzNGcUwrWFlyVFg3ZjlRUjF3Zz09",user_id: @user.id, start_time: Time.now-100, end_time: Time.now+100, count: 1}}
    end
    assert_template 'zooms/new'
  end
  ...
  test "zoom update" do
    ...
    patch zoom_path(@zoom.id), params: { zoom: { title: "testtest",join_url: "https://zoom.us/j/73988743960?pwd=cWRNMzNGcUwrWFlyVFg3ZjlRUjF3Zz09", start_time: time+100, end_time: time+200}}
  ...
  test "other zoom don't update" do
    login_as(@user, scope: :user)
    patch zoom_path(@other_zoom.id), params: { zoom: { title: "testtest", join_url: "https://zoom.us/j/73988743960?pwd=cWRNMzNGcUwrWFlyVFg3ZjlRUjF3Zz09", start_time: time+100, end_time: time+200}}
    assert_template nil
  end
end
