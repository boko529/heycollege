...
class GroupProfilesCreateTest < ActionDispatch::IntegrationTest
  ...
  def setup
    @user1 = users(:user1) # group1に所属、admin == false
    @user2 = users(:user2) # group1に所属、admin == true
    @user4 = users(:user4) # group1,group2に所属、admin == true
    @group1 = groups(:group1)  # user1,user2(admin)が所属,すでにprofile持ってる
    @group2 = groups(:group2)  # user1,user2(admin),user4(admin)が所属,まだprofile持ってない
    @relation2 = user_group_relations(:two) # user2,group1, admin == true
    @relation4 = user_group_relations(:forth) # user1,group1, admin == true
